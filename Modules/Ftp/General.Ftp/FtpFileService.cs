...
namespace General.Ftp
{
    public class FtpFileService : IFtpFileService
    {
        ...
        /// <param name="cancellationToken"></param>
        /// <returns></returns>
        public async Task<FtpOperationResult> CheckFTPServiceAsync(CancellationToken cancellationToken = default)
        {
            var authResult = await GetAuthenticatedClientAsync(cancellationToken);
            if (!authResult.IsSuccess || authResult.Data == null)
            {
            ...
            _logger.LogInformation("FTP服务检测成功，地址：{server}", _connectionOptions.Server);
